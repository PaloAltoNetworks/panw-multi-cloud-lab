...
10. Run the below command to get the Project ID. We will need to set the Project ID before deploying the resources.

```
gcloud projects list --filter='name:qwiklab-MULTICLOUD-LAB' --format="value(projectId)"
```

![](https://lh6.googleusercontent.com/Af_zCaeIQ_s05UFIxyhi2jJjoVOmZcWAUH3gHa4cyyBa4tiStqHvRByjviu7BpVNIdL38MUEzkpYhxQuZLLuL_ZKC-wOrdz3M4LG9wRSDbRNeQWcDoakUf5oBauLqTGZTKbyKCP3BV4waQ5Gww0B5Mk)

11. Set the project using the Project ID displayed in the output of the previous command.

```
gcloud config set project <project id>
```

![](https://lh4.googleusercontent.com/REB3BLv91-XHA8pw7HMFRflBjMxeZ9vzMiQ8_9nTy73uyjXOF0QV5sAkP_FEZvKOgdQ9m7ZmA_VmIAdYH1J8IQts6O3wPimxbEatJOcRV2_3rLbeSXtL4M1TpRrpmjYzajYV-qOHZYiJflQaZf5BWWw)

12. Clone Palo Alto Git repository to build the lab on GCP. Execute below mentioned commands to build the setup on GCP.
...
13. Copy the value of PANORAMA_IP from the output of the setup that you just completed.

14. Run the below command in GCP CloudShell to login to Panorama via CLI using the value copied in the previous step.
...
15. Once logged in, run the following commands on the Panorama prompt. Enter ‘y’ when prompted.
...
16. The Panorama now will be rebooted with the Logging enabled.
...
17. Click on “\[MULTI CLOUD LAB – AWS] Deploy the AWS Cloud Resources”  or open in new tab as shown below
...
18. Now click on “Start Lab” as shown below to start AWS lab.
...
19. If you are on a Mac system, you will be using ‘Terminal’ to connect to the devices via SSH. For this, click on the “Download PEM” link. This will download a file named “qwikLABS-L\*\*\*\*\*-\*\*\*\*\*.pem”. 
20. If you are using a Windows laptop to access this lab, you will need to have a SSH application like PuTTY installed. In this case, click on the “Download PPK” link. This will download a file named “qwikLABS-L\*\*\*\*\*-\*\*\*\*\*.ppk”.
21. Make sure to note the location where the file is downloaded. On a Mac, by default, it should be downloaded to “/Users/&lt;username>/Downloads”.
...
22. To login to the AWS environment, right click on “Open Console’ and “Open link in Incognito window” for Chrome-based browsers. For other browsers, use the appropriate option to open the AWS Console in a new private tab.
23. On the AWS Console, copy over the IAM username and password from the previous tab.
...
25. On the AWS console, If you see a message for ‘new AWS console’, click on ‘switch now’
26. On the search bar type ‘iam’.
27. Click on the link to IAM. A new IAM dashboard window will open.
...
30. Click on default policy to open the IAM policy settings in a new tab.
...
32. Scroll down to the Deny policy and remove two lines (line number 36 and line number 27) listed below 
...
33. Click on ‘Review policy’ at the bottom of the screen
...
34. On the next screen, click on ‘Save changes’
...
35. Account setup is now complete.
...
36. Click on ‘AWS’ on the top left hand corner to navigate to the primary console.
37. Make sure that the region is N.Virginia.
...
39. Wait a few minutes until the effective date changes from ‘Pending’ to date.
...
**Note:** If your account is already subscribed to the VM-Series Marketplace service, then you will immediately get this above screen when you click on “Continue to Subscribe”.

40. From the AWS management Console, launch CloudShell using the icon on the top right side of the console.
...
41. Close out the welcome pop up.
...
42. After the cloudshell is launched, run the following commands. **Make sure to replace _VPN_STATIC_IP_ and _PANORAMA_IP_ with the values noted from the Google Cloud Lab setup output.**
...
43. Once the setup is completed, execute below mentioned commands to copy the output of above setup script into a file.
...
44. Now download the file by going to Actions on the top-right of the page as shown below.
...
45. Enter the individual file path is “/home/cloudshell-user/outputs.json”
...
46. Review the deployed lab environment with the topology diagram.
47. On the AWS Console, on the search bar at the top, type VPC and select VPC from the listed results.
...
48. Review the VPCs that we created.
...
49. Navigate back to the GCP CloudShell tab on your browser. If the CloudShell session got disconnected, please click on “Reconnect”.
50. Upload the outputs.json file (that was downloaded from the AWS CloudShell environment on Step 40) to the GCP CloudShell home directory.
...
51. Run the following commands to complete the setup.
...
52. Search for “VPN” on google console and select “VPN” part of Hybrid Connectivity
...
53. Ensure that both Tunnels (vpn-1 and vpn-2) statuses are GREEN and are up and running.
...
54. On the AWS Console, Navigate to “Site to Site VPN Connections” from the left menu on the VPC Dashboard and click on the single VPN Connection displayed on the page.
...
55. The status of the VPN tunnels here should be up. This might take a few minutes.

56. Login to GCP Spoke VM by clicking on “SSH” next to the VM instance on the GCP console and ping FTP server on AWS with IP 10.1.1.100 using the following command
...
57. On the Google Cloud Console, navigate to the Service Accounts page by selecting “IAM & Admin > Service Accounts” from the left menu pane.
...
58. Identify the “multicloud-lab-sa” service account and click on it.
...
59. On the page that opens up, navigate to the “KEYS” tab and click on “ADD KEY > Create new key”.
...
60. On the popup screen that shows up, ensure that “JSON” is selected and click on “CREATE”.
...
61. If you have taken a note of the Google Cloud Lab Setup output as mentioned in Step 12d, copy-paste the value of PANORAMA_URL on the browser tab to open Panorama. 

62. If not, then follow the below steps to get the Panorama URL.
...
63. Login to Panorama using the below credentials

    1. User – admin
    2. Password – Paloalto@1

64. Navigate to the Panorama tab, then to the Google Cloud Plugin from the left menu.

65. Click on “Google Cloud Plugin > Setup > GCE Service Account” and Click on “Add”
...
66. Move on to Notify Group and click on “Add”.
...
67. Now, under Google Cloud Plugin, click on “Monitoring Definition” and then click on “Add”.
...
68. Commit to Panorama.

69. Once the Commit is completed successfully, we can see the Status on the Monitoring Definition says “Success” and clicking on “Details” will display the “Google Cloud Project ID” that you are working on. It takes around 3 mins to complete.

70. Further steps to create DAG based on GCP subnet here.
...
71. Navigate to the AWS EC2 console, select the “multicloud-lab-ftp-server” instance and click on “Connect”. Ensure that the user is “ec2-user”
...
72. Once you are logged in, run the below command to edit the FTP configuration.
...
73. Now, navigate to the GCP Console and Login to the GCP Spoke VM by clicking on “SSH” on the VM Instance entry on the GCP console.
...
74. Run the below command to download the malware sample.

```
wget <http://www.eicar.org/download/eicar.com.txt>
```

**Note:** This download will not be blocked because we have allowed it as part of the Lab exercise.

75. Login to the FTP server by following the below steps;
