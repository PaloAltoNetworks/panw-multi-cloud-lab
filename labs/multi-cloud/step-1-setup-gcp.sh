...
function deploy_gcp_step_1_lab() {
    PROJECT_ID=$(gcloud projects list --filter='name:qwiklab-MULTICLOUD-LAB' --format="value(projectId)")
    VARS_FILENAME="${HOME}/panw-lab-hybrid-multi-cloud/labs/multi-cloud/gcp/terraform.tfvars"
    ...
    cd "${HOME}/panw-lab-hybrid-multi-cloud/labs/multi-cloud/gcp"
    ...
    # Deploy resources
    echo "Deploying Resources required for Palo Alto Networks Reference Architecture for Zero Trust with VM-Series on GCP"
    terraform apply -auto-approve

    if [ $? -eq 0 ]; then
        echo "GCP Zero Trust Reference Architecture with VM-Series Lab Deployment Completed successfully!"
    else
        echo "GCP Zero Trust Reference Architecture with VM-Series Lab Deployment Failed!"
        exit 1
    fi
}
