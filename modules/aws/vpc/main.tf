...
resource "aws_vpc" "this" {
  tags                 = merge({ Name = "${var.prefix-name-tag}${var.vpc.name}" }, var.global-tags)
...
resource "aws_internet_gateway" "this" {
  tags    = merge({ Name = "${var.prefix-name-tag}${var.vpc.name}-igw" }, var.global-tags)
...
resource "aws_subnet" "this" {
  ...
  tags = merge({ Name = "${var.prefix-name-tag}${var.vpc.name}-${each.value.name}" }, var.global-tags)
  vpc_id = aws_vpc.this.id
}
...
resource "aws_security_group" "this" {
  ...
  tags = merge({ Name = "${var.prefix-name-tag}${each.value.name}" }, var.global-tags)
...
resource "aws_network_interface" "private" {
  ...
  tags = merge({ Name = "${var.prefix-name-tag}${each.value.name}-primary-interface" }, var.global-tags)
}
...
resource "aws_eip" "ec2_eip" {
  ...
  #tags = merge({ Name = "${var.prefix-name-tag}${each.value.name}-eip" }, var.global-tags)
}

resource "aws_eip" "natgw_eip" {
  for_each = var.nat-gateways
